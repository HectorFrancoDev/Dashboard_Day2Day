...
@Component({
  selector: 'app-list-users-cells',
  templateUrl: './list-users-cells.component.html',
  styleUrls: ['./list-users-cells.component.scss']
})
export class ListUsersCellsComponent implements OnInit {
  ...
  public userCelulas = localStorage.getItem('celulas').split(',');
  ...
  loadData() {
    ...
    this.userService.getAllUsers().subscribe(
      (users) => {
        ...
        // this.usersData = users.users.filter((u) => u.state);
        ...
        if (this.userRole === 'LEADER_ROLE') {
          ...
          for (let i = 0; i < users.users.length; i++) {
            ...
            if (user.celulas.length === 0)
              continue;
              


            for (let j = 0; j < user.celulas.length; j++) {
              const celula = user.celulas[j];
              if (this.userCelulas.includes(celula.celula.name)) {
                console.log('108', user.name);
  ...
  showUser(id: string) {
    ...
    if (this.userRole === 'DIRECTOR_ROLE')
      this.router.navigate(['/director/users-cell/' + id]);

    else if (this.userRole === 'LEADER_ROLE')
      this.router.navigate(['/leader/users-cell/' + id]);

    else if (this.userRole === 'LEADER_CAM_ROLE')
      this.router.navigate(['/leader-cam/users-cell/' + id]);

    else if (this.userRole === 'SUPERVISOR_ROLE')
      this.router.navigate(['/supervisor/users-cell/' + id]);

    else if (this.userRole === 'APOYO_DIRECCION_ROLE')
      this.router.navigate(['/apoyo-direccion/users-cell/' + id]);

    else if (this.userRole === 'APOYO_VP_ROLE')
      this.router.navigate(['/apoyo-vp/users-cell/' + id]);

    else
      this.router.navigate(['/vicepresident/users-cell/' + id]);
  ...
  showUserPerformance(id: string) {
    ...
    if (this.userRole === 'DIRECTOR_ROLE')
      this.router.navigate(['/director/users-cell/performance/' + id]);

    else if (this.userRole === 'LEADER_ROLE')
      this.router.navigate(['/leader/users-cell/performance/' + id]);

    else if (this.userRole === 'LEADER_CAM_ROLE')
      this.router.navigate(['/leader-cam/users-cell/performance/' + id]);

    else if (this.userRole === 'SUPERVISOR_ROLE')
      this.router.navigate(['/supervisor/users-cell/performance/' + id]);

    else if (this.userRole === 'APOYO_DIRECCION_ROLE')
      this.router.navigate(['/apoyo-direccion/users-cell/performance/' + id]);

    else if (this.userRole === 'APOYO_VP_ROLE')
      this.router.navigate(['/apoyo-vp/users-cell/performance/' + id]);

    else
      this.router.navigate(['/vicepresident/users-cell/performance/' + id]);
  }
